...
async function searchSku(sku){const params=new URLSearchParams({query:sku,hitsPerPage:'40',distinct:'false'}).toString();const r=await fetch(`https://${APP.toLowerCase()}-dsn.algolia.net/1/indexes/product_variants_v2/query`,{method:'POST',headers:{'content-type':'application/json','x-algolia-application-id':APP,'x-algolia-api-key':SEARCH_KEY,'x-algolia-agent':'Algolia for JavaScript'},body:JSON.stringify({params})});if(!r.ok)return null;const d=await r.json(),hits=Array.isArray(d.hits)?d.hits:[];const exact=hits.filter(x=>skuValues(x).some(v=>norm(v)===norm(sku))||norm(x.sku||x.styleId||x.style_id)===norm(sku));const byProduct=new Map();for(const x of exact){const key=String(x.product_template_id||x.productTemplateId||x.slug||x.objectID||'');if(key&&!byProduct.has(key))byProduct.set(key,x)}return byProduct.size===1?[...byProduct.values()][0]:null}
...
for(let i=0;i<targets.length;i++){
 const p=targets[i],sku=clean(p.style_code);let hit=null;try{hit=await searchSku(sku)}catch{}if(!hit){misses.push({productId:p.product_id,styleCode:sku,reason:'no unique exact GOAT product for SKU'});continue}
 ...
 const detailSku=[...new Set(skuValues(detail))];if(detailSku.length&&!detailSku.some(v=>norm(v)===norm(sku))){misses.push({productId:p.product_id,styleCode:sku,reason:'GOAT detail SKU mismatch'});continue}
