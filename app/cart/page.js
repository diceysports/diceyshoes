...
export default function CartPage(){
  ...
  const priceOf=x=>{const n=Number(x.price);return Number.isFinite(n)&&n>0?n:DEFAULT_PRICE};
  const total=cart.reduce((s,x)=>s+priceOf(x),0);
    ...
    <section className="catalog"><div className="w cartlayout">
      <div className="cartitems">
        {cart.map(x=><div className="cartitem" key={x.id}>
          ...
          <div className="cartcopy"><b>{x.brand}</b><Link href={'/product/'+x.slug}><h3>{x.name}</h3></Link><p>{x.sizing?`${x.sizing}'s US size ${x.size}`:`US size ${x.size}`}</p><strong>{money(priceOf(x))}</strong></div>
      ...
      <aside className="summary">
        ...
        <button className="btn v wide" disabled={!cart.length||busy} onClick={checkout}>{busy?'OPENING STRIPE…':'SECURE CHECKOUT'}</button>
