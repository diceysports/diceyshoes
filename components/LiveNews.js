...
const imageSrc=url=>url&&/^https:\/\//i.test(url)?'/api/shoe-image?url='+encodeURIComponent(url):url||'';
function NewsImage({item}){
  ...
  return <div className="art newsphoto"><img src={imageSrc(item.image)} data-original={item.image} alt={item.title} loading="lazy" onError={e=>{const img=e.currentTarget;if(img.dataset.retry!=='1'){img.dataset.retry='1';img.src=img.dataset.original;return}setFailed(true)}}/></div>;
}
