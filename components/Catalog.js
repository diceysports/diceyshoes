...
export default function Catalog({title='SHOP ALL',eyebrow='DICEY SHOES',filter}){
  ...
  return <main><section className="pagehead"><div className="w"><div className="k">{eyebrow}</div><h1>{brand||title}</h1><p>{loading?'Loading the full catalog…':`${list.length.toLocaleString()} shoes in this view · ${products.length.toLocaleString()} total pairs · every pair has an image · unavailable prices display as TBD.`}</p></div></section><section className="catalog"><div className="w">
    ...
    <div className="tools"><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search 9,000 shoes, models or brands" autoFocus={title==='SEARCH'}/><select value={category} onChange={e=>setCategory(e.target.value)}>{categories.map(c=><option value={c} key={c}>{c==='All'?'All categories':c}</option>)}</select><select value={sort} onChange={e=>setSort(e.target.value)}><option value="featured">Featured</option><option value="low">Price: low to high</option><option value="high">Price: high to low</option></select></div>
    <div className="catalogCount">Showing {Math.min(shown.length,list.length).toLocaleString()} of {list.length.toLocaleString()}{brand?` · ${brand}`:''}{category!=='All'?` · ${category}`:''}</div><div className="grid">{shown.map(p=><ProductCard p={p} key={p.slug}/>)}</div>{visible<list.length&&<div className="loadMoreWrap"><button className="btn v" onClick={()=>setVisible(v=>v+PAGE_SIZE)}>LOAD MORE SHOES</button></div>}{!list.length&&!loading&&<div className="empty">No shoes match these filters.</div>}
  </div></section></main>;
}
