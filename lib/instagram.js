...
export function readImageSize(buffer){
  ...
  if(bytes.length>12&&String.fromCharCode(...bytes.slice(0,4))==='RIFF'&&String.fromCharCode(...bytes.slice(8,12))==='WEBP'){
    ...
    if(chunk==='VP8X')return{width:1+((bytes[24]<<16)|(bytes[25]<<8)|bytes[26]),height:1+((bytes[27]<<16)|(bytes[28]<<8)|bytes[29]),format:'webp'};
    if(chunk==='VP8 ')return{width:view.getUint16(26,true)&0x3fff,height:view.getUint16(28,true)&0x3fff,format:'webp'};
    if(chunk==='VP8L'){const b=bytes.slice(21,25);return{width:1+(((b[1]&0x3f)<<8)|b[0]),height:1+(((b[3]&0xf)<<10)|(b[2]<<2)|((b[1]&0xc0)>>6)),format:'webp'}}
  }
...
export async function imageDimensions(url,{fetchImpl=globalThis.fetch,timeoutMs=10000,bytes=65536}={}){
  const controller=new AbortController();
  const timer=setTimeout(()=>controller.abort(),timeoutMs);
  try{
    const res=await fetchImpl(url,{headers:{Range:`bytes=0-${bytes-1}`,'User-Agent':'Mozilla/5.0 DiceyShoes/1.0'},redirect:'follow',signal:controller.signal});
    clearTimeout(timer);
    if(!res.ok&&res.status!==206)return{ok:false,status:res.status};
    const size=readImageSize(new Uint8Array(await res.arrayBuffer()));
    return size?{ok:true,status:res.status,...size}:{ok:false,status:res.status,reason:'unrecognised image header'};
  }catch(e){
    clearTimeout(timer);
...
export async function tokenStatus(client){
  if(!client.base.includes('graph.instagram.com'))return{checked:false,reason:'Token refresh check only applies to Instagram Login tokens.'};
  try{
    const json=await request(client,'refresh_access_token',{params:{grant_type:'ig_refresh_token'}});
    const days=Math.round(Number(json.expires_in||0)/86400);
    return{checked:true,expiresInDays:days,healthy:days>14};
  }catch(e){
    return{checked:false,reason:e.message,healthy:false};
  }
}
